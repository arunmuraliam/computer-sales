...
&nbsp;</p>
  ...
  <tr>
    ...
    <td colspan="2" rowspan="3" valign="top"><form name="form2" method="post" action="receiveoldprd3.php">
		...
		<?php $receivedate=date("y")."-".date("m")."-".date("d"); ?>
          <!--DWLayoutTable-->
          <tr>
            <td width="215">Request Status </td>
            <td width="201"><?php echo "$rstatus"; ?></td>
          </tr>
          <tr>
          ...
          <tr>
            <td colspan="2"><div align="center">
                <textarea name="t1" id="t1"><?php echo "$rdetails"; ?></textarea>
            </div></td>
          </tr>
          <tr>
            <td height="38" colspan="2"><div align="center">
                <?php 
				  $s="update custsendreceiveproduct set rstatus='$rstatus',rdetails='$rdetails',rdate='$receivedate',staffid='$staffid' where reqno='$reqno'";
				  if(mysql_query($s))
				  {
				    echo "<b>Inserted Received Details ";
				  }
				  ...
				  while($row=mysql_fetch_array($s))
				  {
					...
					$ss=mysql_query("select *from oldstock where oldpid='$oldpid' and model='$model'");
					if(mysql_num_rows($ss)==0)
					{
						$sss="insert into oldstock values('$oldpid','$model','$qty')";
					}
					else
					{
						$sss="update oldstock set cstock=cstock+$qty where oldpid='$oldpid' and model='$model'";
					}
					mysql_query($sss);
